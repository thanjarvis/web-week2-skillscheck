...
const me = {
	...
	age: 22,
	greeter: function(){
		return `Hello! My name is ${me.firstName} and I live in ${this.state}`
	}
}
...
const doubled = myNumbers.map((e, i, a) => {
	return e*2
})
...
const filtered = myNumbers.filter((e, i, a) => {
	return e > 100
})
...
const total = myNumbers.reduce((t, e, i, a) => {
	return e + t
})
...
const forTheLoveOfGeorge = notGeorge.map((e, i, a) => {
	return 'George'
})
...
const totallyAwesome = people.reduce((total, element, index, array) =>{
	return total + (array[index].awesomeLevel)
